...
public class SBPR extends SocialRecommender {
	...
	private Map<Integer, List<Integer>> DP;
	...
	protected void initModel() throws Exception {
		...
		for (int u = 0, um = trainMatrix.numRows(); u < um; u++) {
			...
			// SPu
//			System.out.println(socialMatrix.getColumns(u));
			List<Integer> trustedUsers = socialMatrix.getColumns(u);
			
//			System.out.println("trust"+trustedUsers);
//			System.out.println("distrust"+distrustedUsers);
			...
			List<Integer> distrustedUsers = socialMatrix.getColumnsdis(u);
			List<Integer> disitems = new ArrayList<>();
			for (int v : distrustedUsers) {
				if (v >= um) // distrust v
					continue;
				...
				for (int j : vRatedItems) {
					// v's rated items
					if (!uRatedItems.contains(j) && !disitems.contains(j) && !items.contains(j)) // if not rated by user u and not already added to item list
						disitems.add(j);
				}
			}
	...
	protected void buildModel() throws Exception {
		...
		for (int iter = 1; iter <= numIters; iter++) {
			...
			for (int s = 0, smax = numUsers * 100; s < smax; s++) {
				...
				//DPu
				List<Integer> DPu = DP.get(u);
				
				// j
				do {
					j = Randoms.uniform(numItems);
				} while (ratedItems.contains(j) || SPu.contains(j) || DPu.contains(j));

				double xuj = predict(u, j);

				if (SPu.size() > 0 && DPu.size() == 0) {
					// if having social neighbors
					int k = Randoms.random(SPu);//用户u的好友评价过的商品
				...
				if (SPu.size() > 0 && DPu.size() > 0) {
					// if having social neighbors
					int k = Randoms.random(SPu);//用户u的好友评价过的商品
					int z = Randoms.random(DPu);
					double xuk = predict(u, k);
					double xuz = predict(u, z);
					...
					double vals = -Math.log(g(xuik)) - Math.log(g(xukj)) - Math.log(g(xujz));
					loss += vals;

					double cik = g(-xuik), ckj = g(-xukj),cjz = g(-xujz);
					...
					double bj = itemBias.get(j);
					itemBias.add(j, lRate * (-ckj + cjz - regB * bj));
					loss += regB * bj * bj;
					
					double bz = itemBias.get(z);
					itemBias.add(z, lRate * (-cjz - regB * bz));
					loss += regB * bz * bz;
					...
					for (int f = 0; f < numFactors; f++) {
						...
						double qif = Q.get(i, f), qkf = Q.get(k, f);
						double qjf = Q.get(j, f);
						double qzf = Q.get(z, f);
						
						double delta_puf = cik * (qif - qkf) / (1 + suk) + ckj * (qkf - qjf) + cjz * (qjf - qzf);
						P.add(u, f, lRate * (delta_puf - regU * puf));
...
//						Q.add(j, f, lRate * (ckj * (-puf) - regI * qjf));
						Q.add(j, f, lRate * (ckj * (-puf) + cjz * (puf)- regI * qjf));
						
						Q.add(z, f, lRate * (cjz * (-puf) - regI * qzf));

						loss += regU * puf * puf + regI * qif * qif;
						loss += regI * qkf * qkf + regI * qjf * qjf + regI * qzf * qzf;
					}
				} 
				if (SPu.size() == 0 && DPu.size() > 0) {
					// if having social neighbors
					int z = Randoms.random(DPu);//用户u的好友评价过的商品
					double xuz = predict(u, z);

//					SparseVector dTu = socialMatrix.disrow(u);
//					
					double xuij = xui - xuj;
					double xujz  = xuj - xuz;
					
					double vals = -Math.log(g(xuij)) - Math.log(g(xujz));
					loss += vals;
					...
					double bj = itemBias.get(j);
					itemBias.add(j, lRate * (-cij + cjz - regB * bj));
					loss += regB * bj * bj;
					...
					for (int f = 0; f < numFactors; f++) {
						...
						P.add(u, f, lRate * (cij * (qif - qjf) + cjz * (qjf - qzf) - regU * puf));
						Q.add(i, f, lRate * (cij * puf - regI * qif));
						Q.add(j, f, lRate * (cij * (-puf) + cjz * (puf)- regI * qjf));
						Q.add(z, f, lRate * (cjz * (-puf) - regI * qzf));
						
						loss += regU * puf * puf + regI * qif * qif + regI * qjf * qjf + regI * qzf * qzf;
					}
